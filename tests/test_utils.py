...
def test_calculate_frame_at(trex):
    ...
    assert frame_ps2.subbounds().sizes["subframe"] == 8
    ...
    assert sc.allclose(bw_min, 1.44 * sc.Unit("Å"), rtol=sc.scalar(0.05))
    assert sc.allclose(bw_max, 3.28 * sc.Unit("Å"), rtol=sc.scalar(0.05))

    frame_mon2 = calculate_frame_at("Monitor 2", trex)
    assert frame_mon2.subbounds().sizes["subframe"] == 8
