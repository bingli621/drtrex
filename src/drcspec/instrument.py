...
from drcspec.components import Source, Chopper, Monitor, Detector
from drcspec.components.utils import calculate_frame_at, acceptance_paths
from drcspec.params import chopper_params, monitor_params, detector_params

if TYPE_CHECKING:
    from drcspec.components.utils import SubframeVortex
...
class Instrument(object):
    ...
    def __init__(
        ...
        f_M=sc.scalar(14.0, unit="Hz"),
        n: int = 1,
        phase_bw3=sc.scalar(0.0, unit="deg"),
        t_offset=sc.scalar(0.0, unit="s"),
        source=Source(facility="ess-cspec", neutrons=1_000_000, pulses=1),  # type: ignore
    ) -> None:
    ...
    def __str__(self) -> str:
        cspec_str = f"CSPEC running with cententral wavelength = {self.wavelength.value:.2f} Å\n"
        ps_str = f"PSC frequency =  {self.f_M.value / 2} Hz, "
        rrm_str = f"RRM frequency = {self.f_M.value / self.n} Hz, "
        m_str = f"MC frequency = {self.f_M.value} Hz"

        return cspec_str + ps_str + rrm_str + m_str
